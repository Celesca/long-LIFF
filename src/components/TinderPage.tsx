...
const TinderPage: React.FC = () => {
        ...
        <div className="min-h-screen flex flex-col items-center justify-center p-6">
          <div className="text-center space-y-4">
            <div className="relative w-16 h-16 mx-auto">
              <div className="absolute inset-0 border-3 border-[#DDEAF3] rounded-full" />
      ...
      <Layout showHeader showBackButton headerTitle="สำรวจ" backgroundVariant="tinder">
        <div className="min-h-[80vh] flex flex-col items-center justify-center p-6">
          <div className="text-center space-y-4 max-w-md">
            <div className="w-16 h-16 mx-auto bg-[#EF476F]/10 rounded-2xl flex items-center justify-center">
              <svg className="w-8 h-8 text-[#EF476F]" fill="none" stroke="currentColor" strokeWidth={1.5} viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" d="M12 9v3.75m9-.75a9 9 0 11-18 0 9 9 0 0118 0zm-9 3.75h.008v.008H12v-.008z" />
              </svg>
            </div>
            <h2 className="text-lg font-bold text-[#17324D]">มีบางอย่างผิดพลาด</h2>
            <p className="text-[#8AA0B3] text-sm">{error}</p>
            <button
              onClick={() => selectedLocation ? fetchPlaces(selectedLocation) : setShowLocationModal(true)}
              className="mt-2 bg-[#FF6B4A] text-white py-2.5 px-6 rounded-xl font-semibold text-sm active:scale-95 transition-transform"
            >
              ลองใหม่
            </button>
          </div>
      ...
      <Layout showHeader showBackButton headerTitle="สำรวจ POI จริง" backgroundVariant="tinder">
        <div className="min-h-[80vh] flex flex-col items-center justify-center p-6">
          <div className="text-center space-y-4 max-w-sm">
            <div className="w-16 h-16 mx-auto bg-[#0077B6]/10 rounded-2xl flex items-center justify-center">
              <svg className="w-8 h-8 text-[#0077B6]" fill="none" stroke="currentColor" strokeWidth={1.6} viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" d="M9 6.75V15m6-6v8.25m.503 3.498 4.875-2.437c.381-.19.622-.58.622-1.006V4.82c0-.836-.88-1.38-1.628-1.006l-3.869 1.934c-.317.159-.69.159-1.006 0L9.503 3.252a1.125 1.125 0 0 0-1.006 0L3.622 5.689C3.24 5.88 3 6.27 3 6.695V19.18c0 .836.88 1.38 1.628 1.006l3.869-1.934c.317-.159.69-.159 1.006 0l4.994 2.497c.317.158.69.158 1.006 0Z" />
              </svg>
            </div>
            <h2 className="text-xl font-bold text-[#17324D]">เลือกจุดเริ่มค้นหา</h2>
            <p className="text-sm text-[#4F6F87]">ปักหมุดตำแหน่งที่คุณอยากไป แล้วระบบจะดึง POI จริงจาก places.json รอบตำแหน่งนั้นมาให้ปัด</p>
            <button
              onClick={() => setShowLocationModal(true)}
              className="w-full rounded-xl bg-gradient-to-r from-[#FF6B4A] to-[#E85336] px-6 py-3 text-sm font-semibold text-white shadow-md"
            >
              ปักหมุดค้นหา POI
            </button>
          </div>
      ...
      <Layout showHeader headerTitle="เสร็จสิ้น!" showCoinCounter backgroundVariant="tinder">
        <div className="min-h-[80vh] flex flex-col items-center justify-center p-6">
          <div className="text-center space-y-5 max-w-sm">
            <div className="w-20 h-20 mx-auto bg-[#FF6B4A] rounded-2xl flex items-center justify-center shadow-md">
            ...
            <div className="card-surface p-4">
              <div className="flex items-center justify-center space-x-2">
                <svg className="w-6 h-6 text-[#FF6B4A]" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M11.645 20.91l-.007-.003-.022-.012a15.247 15.247 0 01-.383-.218 25.18 25.18 0 01-4.244-3.17C4.688 15.36 2.25 12.174 2.25 8.25 2.25 5.322 4.714 3 7.688 3A5.5 5.5 0 0112 5.052 5.5 5.5 0 0116.313 3c2.973 0 5.437 2.322 5.437 5.25 0 3.925-2.438 7.111-4.739 9.256a25.175 25.175 0 01-4.244 3.17 15.247 15.247 0 01-.383.219l-.022.012-.007.004-.003.001a.752.752 0 01-.704 0l-.003-.001z" />
                </svg>
                <span className="text-2xl font-bold text-[#FF6B4A]">{likedPlaces.length}</span>
              </div>
              <div className="text-sm text-[#8AA0B3] mt-1">สถานที่ที่บันทึก</div>
            </div>
            ...
            <div className="space-y-2.5 w-full">
              ...
              <button
                ...
                className="flex items-center justify-center w-full bg-[#FF6B4A] text-white py-3 px-6 rounded-xl font-semibold text-sm active:scale-95 transition-transform"
              >
                <svg className="w-4.5 h-4.5 mr-2" fill="none" stroke="currentColor" strokeWidth={1.8} viewBox="0 0 24 24">
              ...
              <button
                ...
                className="flex items-center justify-center w-full bg-white text-[#4F6F87] py-3 px-6 rounded-xl font-semibold text-sm border border-[#DDEAF3] active:scale-95 transition-transform"
              >
                <svg className="w-4.5 h-4.5 mr-2" fill="none" stroke="currentColor" strokeWidth={1.8} viewBox="0 0 24 24">
  ...
  return (
      ...
      <div className="min-h-screen flex flex-col pb-24 lg:pb-32">
        {/* Header */}
        <div className="sticky top-0 z-30 flex items-center justify-between border-b border-[#DDEAF3]/50 bg-white/80 px-4 py-3 backdrop-blur-xl lg:px-8">
          <Link
            to="/"
            className="flex items-center justify-center w-10 h-10 rounded-full bg-[#EDF6FB] active:scale-95 transition-transform"
            aria-label="Go back"
          >
            <svg className="w-5 h-5 text-[#4F6F87]" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
          ...
          <Link
            to="/gallery"
            className="relative flex items-center justify-center w-10 h-10 rounded-full bg-[#EDF6FB] active:scale-95 transition-transform"
          ...
          >
            ...
            {likedPlaces.length > 0 && (
              <span className="absolute -top-0.5 -right-0.5 w-4.5 h-4.5 bg-[#FF6B4A] text-white rounded-full text-[10px] flex items-center justify-center font-bold shadow-sm">
        ...
        <div className="bg-white/40 px-4 py-2.5 backdrop-blur-sm lg:px-8">
          ...
          <div className="flex items-center space-x-3">
            <div className="flex-1 h-1 bg-[#DDEAF3] rounded-full overflow-hidden">
              <div
            ...
            <span className="text-[11px] font-semibold text-[#8AA0B3] whitespace-nowrap bg-[#EDF6FB] px-2 py-0.5 rounded-full">
        ...
        {/* Action Buttons */}
        <div className="fixed bottom-0 left-0 right-0 bg-white/90 backdrop-blur-xl border-t border-[#DDEAF3]/50 safe-area-bottom lg:left-72">
          <div className="flex items-center justify-center space-x-5 py-4 px-6 max-w-lg mx-auto">
            {/* Skip */}
            <button
              onClick={() => handleSwipe('left')}
              className="w-14 h-14 bg-white rounded-full shadow-md border border-[#DDEAF3] flex items-center justify-center active:scale-90 transition-transform"
              aria-label="Skip"
            >
              <svg className="w-6 h-6 text-[#8AA0B3]" fill="none" stroke="currentColor" strokeWidth={2.5} viewBox="0 0 24 24">
            ...
            <button
              ...
              className="w-11 h-11 bg-[#0077B6] rounded-full shadow-md flex items-center justify-center active:scale-90 transition-transform"
            ...
            <button
              ...
              className="w-14 h-14 bg-[#FF6B4A] rounded-full shadow-md flex items-center justify-center active:scale-90 transition-transform"
              aria-label="Like"
            >
          ...
          <div className="text-center pb-2.5 -mt-1">
            <p className="text-[11px] text-[#8AA0B3]">ปัดซ้ายเพื่อข้าม · ปัดขวาเพื่อบันทึก</p>
          </div>
        </div>
